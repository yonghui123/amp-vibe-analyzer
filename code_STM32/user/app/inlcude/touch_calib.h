 ...
 *
 * 持久化默认写入 SD：Config/touch_calib.bin（见 path_layout.h）。
 */
...
typedef enum {
    ...
    TOUCH_CALIB_SRC_FLASH,    /**< 旧版片内 Flash（只读兼容） */
    TOUCH_CALIB_SRC_V5_PARAM,
    TOUCH_CALIB_SRC_RAM,
    TOUCH_CALIB_SRC_SD,       /**< SD: Config/touch_calib.bin */
} touch_calib_src_t;

void TouchCalib_Init(void);
bool TouchCalib_IsValid(void);

/** 是否从非易失存储加载（SD 或旧 Flash） */
