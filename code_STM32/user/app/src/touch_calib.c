...

{
    ...
    } else if (*px >= (int32_t)LCD_GetWidth()) {

        *px = (int32_t)LCD_GetWidth() - 1;
    ...
    } else if (*py >= (int32_t)LCD_GetHeight()) {

        *py = (int32_t)LCD_GetHeight() - 1;
...

{
    ...
    crc = touch_calib_crc16((const uint8_t *)&stored->params, sizeof(stored->params));
    ...
    s_source = TOUCH_CALIB_SRC_FLASH;
...

{
    ...
    const touch_v5_param_t *v5 = (const touch_v5_param_t *)V5_PARAM_FLASH_ADDR;
    ...
    if (TouchCalib_IsValid()) {
        ...
        if (s_source == TOUCH_CALIB_SRC_FLASH) {

            snprintf(buf, len,

                     "OK Flash@E0000 magic=%08lX",
    ...
    snprintf(buf, len,
             ...
             "NONE E0000=%08lX C000=%08lX",

             (unsigned long)tp4->magic, (unsigned long)v5->ParamVer);
...

{
    ...
    uint32_t addr = TOUCH_CALIB_FLASH_ADDR;
    ...
    store.crc = touch_calib_crc16((const uint8_t *)&store.params, sizeof(store.params));
...

    {
        ...
        const touch_calib_store_t *verify =
        ...
        if (verify->magic != TOUCH_CALIB_MAGIC

            && verify->magic != TOUCH_CALIB_MAGIC_LEGACY) {

            goto out;

        }

        crc = touch_calib_crc16((const uint8_t *)&verify->params,

                                sizeof(verify->params));
