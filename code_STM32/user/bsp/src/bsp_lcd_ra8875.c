...
static volatile uint16_t s_usTouchX, s_usTouchY;	/* 触摸屏ADC值 */
static volatile uint8_t s_ucRA8875Busy = 0;			/* 忙标志, 其他模块通过 RA8875_IsBusy()函数判断忙 */
static volatile uint8_t s_reg_40H;		/* 0X40寄存器的值 */
static volatile uint8_t s_reg_22H;		/* 0X22寄存器的值 */
...
void RA8875_TouchInit(void)
{
	...
	*/
	RA8875_WriteReg(0x71, (0 << 6) | (0 << 5) | (1 << 2));	/* 自动模式 */

	/* 较快 ADC 取样，降低触摸延迟 (原 4096 周期过慢) */
	RA8875_WriteReg(0x70, (1 << 7) | (1 << 4) | (0 << 3) | (0 << 0));
}
...
{	
	...
	RA8875_REG = 0x02;	/* 设置寄存器地址 */
	RA8875_RAM = _usColor;	/* 设置寄存器地址 */
#else	
	RA8875_SetCursor(_usX, _usY);
