...
static void LCD_Hard_Reset(void);	/* 硬件复位 （STM32-V5 无需），针对其他GPIO控制LCD复位的产品 */
...
{
	LCD_Hard_Reset();	/* 硬件复位 （STM32-V5 无需），针对其他GPIO控制LCD复位的产品 */
	/* FSMC重置后必须加延迟才能访问总线设备  */
	HAL_Delay(20);
	g_ChipID = IC_8875;
	RA8875_InitHard();	/* 初始化RA8875芯片 */
	LCD_ClrScr(CL_BLACK);	/* init test pattern; LVGL will overwrite */
...
/* Draw a small check mark inside a checkbox area.
 * _usX/_usY is the origin (top-left) of an inner area (typically called with x+3, y+3)
 */
void LCD_DrawCheckMark(uint16_t _usX, uint16_t _usY, uint16_t _usColor)
{
	/* Draw two lines making a 'V' shaped tick. Coordinates tuned for 24x24 checkbox. */
	/* Left short stroke */
	LCD_DrawLine(_usX + 3, _usY + 11, _usX + 8, _usY + 16, _usColor);
	LCD_DrawLine(_usX + 3, _usY + 12, _usX + 8, _usY + 17, _usColor);
	/* Right long stroke */
	LCD_DrawLine(_usX + 8, _usY + 16, _usX + 18, _usY + 6, _usColor);
	LCD_DrawLine(_usX + 9, _usY + 16, _usX + 19, _usY + 6, _usColor);
}
...
void LCD_DrawCheckBox(CHECK_T *_pCheckBox)
{
	...
	if (_pCheckBox->Checked)
	{
	  ...
	  font.FontCode = FC_ST_16;
		...
		font.Space = 0;
		x = _pCheckBox->Left;
		/* Draw tick mark using pixels to avoid font/encoding issues */
		LCD_DrawCheckMark(x + 3, _pCheckBox->Top + 3, CHECK_BOX_CHECKED_COLOR);
	}
#else
	if (g_ChipID == IC_8875)
	{
...
void LCD_DrawLabel2(LABEL_T *_pLabel, uint16_t _Color, uint8_t _Arc)
{
		...
		//uint8_t _Arc = 10;
		/* 画弧边 */
		//LCD_DrawCircle(_pBtn->Left + _Arc, _pBtn->Top + _Arc, _Arc, BUTTON_BORDER_COLOR);
		RA8875_FillCircle(_pLabel->Left + _Arc, _pLabel->Top + _Arc,  _Arc,  _Color);					/* 左上圆 */
		RA8875_FillCircle(_pLabel->Left + _pLabel->Width - (_Arc + 1) , _pLabel->Top + _Arc, _Arc, _Color);			/* 右上圆 */
		RA8875_FillCircle(_pLabel->Left + _Arc, _pLabel->Top + _pLabel->Height - (_Arc + 1), _Arc, _Color);			/* 左下圆 */
		RA8875_FillCircle(_pLabel->Left + _pLabel->Width - (_Arc + 1), _pLabel->Top + _pLabel->Height - (_Arc + 1), _Arc, _Color);		/* 右下圆 */
		
		LCD_Fill_Rect(_pLabel->Left + _Arc, _pLabel->Top, _pLabel->Height, _pLabel->Width - 2 * _Arc, _Color);		/* 铺底色 */
		LCD_Fill_Rect(_pLabel->Left, _pLabel->Top + _Arc, _pLabel->Height - 2 * _Arc, _pLabel->Width, _Color);		
	
		/* 文字居中 */
		len = strlen(_pLabel->pCaption);

		if (_pLabel->Font->FontCode == FC_ST_16)
		{
			fwidth = 8;		/* 字体宽度 */
		}
		else if (_pLabel->Font->FontCode == FC_ST_12)
		{
			fwidth = 6;		/* 字体宽度 */
		}
		else if (_pLabel->Font->FontCode == FC_ST_16)
		{
			fwidth = 8;		/* 字体宽度 */
		}
		else if (_pLabel->Font->FontCode == FC_ST_24)
		{
			fwidth = 12;
		}
		else if (_pLabel->Font->FontCode == FC_ST_32)
		{
			fwidth = 16;
		}
		
		x = _pLabel->Left + _pLabel->Width / 2 - (len * fwidth) / 2;
		y = _pLabel->Top + _pLabel->Height / 2 - fwidth;

		LCD_DispStr(x, y, _pLabel->pCaption, _pLabel->Font);
}
...
void LCD_DrawButton2(BUTTON_T *_pBtn, uint8_t _Arc)
{
		...
		if (_pBtn->Focus == 1)
		{
			RA8875_FillCircle(_pBtn->Left + _Arc, _pBtn->Top + _Arc,  _Arc,  CL_BLUE7);					/* 左上圆 */
			RA8875_FillCircle(_pBtn->Left + _pBtn->Width - (_Arc + 1) , _pBtn->Top + _Arc, _Arc, CL_BLUE7);			/* 右上圆 */
			RA8875_FillCircle(_pBtn->Left + _Arc, _pBtn->Top + _pBtn->Height - (_Arc + 1), _Arc, CL_BLUE8);			/* 左下圆 */
			RA8875_FillCircle(_pBtn->Left + _pBtn->Width - (_Arc + 1), _pBtn->Top + _pBtn->Height - (_Arc + 1), _Arc, CL_BLUE8);		/* 右下圆 */
			
			LCD_Fill_Rect(_pBtn->Left + _Arc, _pBtn->Top, _pBtn->Height, _pBtn->Width - 2 * _Arc, CL_BLUE8);		/* 铺底色 */
			LCD_Fill_Rect(_pBtn->Left, _pBtn->Top + _Arc, _pBtn->Height - 2 * _Arc, _pBtn->Width, CL_BLUE8);		
			
			LCD_Fill_Rect(_pBtn->Left + _Arc, _pBtn->Top, _pBtn->Height / 2, _pBtn->Width - 2 * _Arc, CL_BLUE7);	/* 画阴影对比色 */		
		}
		else
		{
			/* 画弧边 */
			//LCD_DrawCircle(_pBtn->Left + _Arc, _pBtn->Top + _Arc, _Arc, BUTTON_BORDER_COLOR);
			RA8875_FillCircle(_pBtn->Left + _Arc, _pBtn->Top + _Arc,  _Arc,  CL_GREY5);					/* 左上圆 */
			RA8875_FillCircle(_pBtn->Left + _pBtn->Width - (_Arc + 1) , _pBtn->Top + _Arc, _Arc, CL_GREY5);			/* 右上圆 */
			RA8875_FillCircle(_pBtn->Left + _Arc, _pBtn->Top + _pBtn->Height - (_Arc + 1), _Arc, CL_GREY5);			/* 左下圆 */
			RA8875_FillCircle(_pBtn->Left + _pBtn->Width - (_Arc + 1), _pBtn->Top + _pBtn->Height - (_Arc + 1), _Arc, CL_GREY5);		/* 右下圆 */
			
			LCD_Fill_Rect(_pBtn->Left + _Arc, _pBtn->Top, _pBtn->Height, _pBtn->Width - 2 * _Arc, CL_GREY5);		/* 铺底色 */
			LCD_Fill_Rect(_pBtn->Left, _pBtn->Top + _Arc, _pBtn->Height - 2 * _Arc, _pBtn->Width, CL_GREY5);		
			
			
			LCD_Fill_Rect(_pBtn->Left + _Arc, _pBtn->Top, _pBtn->Height / 2, _pBtn->Width - 2 * _Arc, CL_GREY6);	/* 画阴影对比色 */
			//LCD_Fill_Rect(_pBtn->Left, _pBtn->Top + _Arc, _pBtn->Height / 2 - _Arc, _pBtn->Width, CL_GREY6);	
		}
		...
		else if (_pBtn->Font->FontCode == FC_ST_16)
		{
			fwidth = 8;		/* 字体宽度 */
		}
		else if (_pBtn->Font->FontCode == FC_ST_24)
		{
			fwidth = 12;
		}
		else if (_pBtn->Font->FontCode == FC_ST_32)
		{
			fwidth = 16;
		}
